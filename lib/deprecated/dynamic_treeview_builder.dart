...
class _TreeState extends State<Tree> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        //   ),
        // ),
        body: FutureBuilder<List<BaseData>>(
            future: formattedPosts,
            builder: ((context, snapshot) {
              if (snapshot.hasData) {
                final extras = snapshot.data!.first.getExtraData();
                //final opPost = extras['postInfo'].num_.toString();
...
Future<List<BaseData>> formatPosts() async {
  ...
  thread.posts?.forEach((post) {
    ...
    for (var parent in parents) {
      final formattedPost = DataModel(
        id: post.num_,
        name: post.num_.toString(),
        parentId: parent,
        extras: {'postInfo': post},
      );
  ...
  return formattedPosts.take(100).toList();
